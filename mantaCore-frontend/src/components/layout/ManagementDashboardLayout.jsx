                ...
                <div className="mb-12">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        >
                            <div className="mt-6">
                                <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-white/30 overflow-hidden hover:shadow-2xl transition-all duration-300 overflow-x-auto">
                                    <div className="px-6 py-6 border-b border-teal-100 bg-gradient-to-r from-teal-50 to-white">
                                        <table className="w-full border-collapse text-sm">
                                            <thead>
                                                <tr className="bg-gradient-to-r from-teal-50/90 via-teal-50/30 to-teal-50/90 shadow-sm">
                                                    <th className="text-left py-3 px-4 font-medium text-teal-700">Date</th>
                                                    <th className="text-left py-3 px-4 font-medium text-teal-700">Category</th>
                                                    <th className="text-left py-3 px-4 font-medium text-teal-700">Author</th>
                                                    <th className="text-left py-3 px-4 font-medium text-teal-700">Amount</th>
                                                    <th className="text-left py-3 px-4 font-medium text-teal-700">Status</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-teal-100">
                                                {recentPurchases.length === 0 ? (
                                                    ...
                                                    recentPurchases.slice(0, 5).map((purchase, index) => (
                                                        <tr key={purchase.purchaseID || index} className="hover:bg-teal-50/50">
                                                            <td className="py-3 px-4 font-medium">
                                                                {new Date(purchase.date).toLocaleDateString('id-ID')}
                                                            </td>
                                                            <td className="py-3 px-4">{purchase.items.map(p => p.item?.category).join(', ') || 'Uncategorized'}</td>
                                                            <td className="py-3 px-4">{purchase.user.username || purchase.authorName || 'Unknown'}</td>
                                                            <td className="py-3 px-4">{formatRupiah(purchase.amount || 0)}</td>
                                                            <td className="py-3 px-4">
                                                                <span className={`px-2 py-1 capitalize ${getStatusColor(purchase.status) || 'bg-gray-100 text-gray-700'
                                ...
                                <div className="mt-6 text-center">
                                    <Link
                                        href="/management/purchase-request"
                        ...
                        >
                            <div className="mt-6">
                                <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-white/30 overflow-hidden hover:shadow-2xl transition-all duration-300 overflow-x-auto">
                                    <div className="px-6 py-6 border-b border-rose-100 bg-gradient-to-r from-rose-50 to-white">
                                        <table className="w-full border-collapse text-sm">
                                            <thead>
